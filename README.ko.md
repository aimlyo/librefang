...
>
> 코드베이스, 바이너리, crate 이름, 설정 경로는 전환 기간 동안 호환성을 위해 `openfang` 상태로 유지됩니다. LibreFang은 다른 프로젝트 거버넌스를 채택합니다: 우리는 커뮤니티 기여를 적극적으로 수용하고, 공개적으로 검토하며, 일반적인 GitHub 플로우로 병합하고, 코드를改编할 때 기여자 표시를 유지합니다.
