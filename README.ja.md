...
>
> コードベース、バイナリ、crate名、設定パスは移行期間中は互換性のために`openfang`のままです。LibreFangは異なるプロジェクトガバナンスを行っています：私たちはコミュニティのコントリビューションを積極的に受け入れ、公開でレビューし、通常のGitHubフローでマージし、コードを改変したときはコントリビューターの帰属を保持します。
